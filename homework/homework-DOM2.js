...
document.body.append(createNewsEl());
function createNewsEl() {
  const containerNews = document.createElement("article");
  const headerEl = document.createElement("header");
  const titleEl = document.createElement("h2");
  const btnHeart = document.createElement("button");
  const heartIcon = document.createElement("i");

  containerNews.classList.add("container");
  ...
  document.body.append(containerNews);
  containerNews.append(headerEl);
  headerEl.append(titleEl, btnHeart);
  btnHeart.append(heartIcon);
  ...
  crossEl.classList.add("fa-solid");
  crossEl.classList.add("fa-xmark");

  containerNews.append(mainEl);
  mainEl.append(categoryEl, textEl, dateEl, btnDelete);
  btnDelete.append(crossEl);

  btnDelete.addEventListener("click", () => {
    containerNews.remove();
  });

  btnHeart.addEventListener("click", function createLike() {
    heartIcon.classList.toogle("heartIconClick");
  });
