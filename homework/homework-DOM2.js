...
function createNewsCard(item) {
  ...
  const newsParagraph = createParagraph(item);
...
function createNewsInfo({ headerBgSrc, title }) {
  ...
  newsInfo.style.backgroundImage = `url(${headerBgSrc})`;
...
function createParagraph({ body }) {
  ...
  newsParagraph.classList.add("newsParagraph");
  newsParagraph.textContent = body;
...
function createLikeIcon() {
  ...
  likeIcon.classList.add("heartinactive");
  likeIcon.classList.add("heartactive");
  likeIcon.addEventListener("click", function (e) {
    e.target.classList.toggle("heartinactive");
